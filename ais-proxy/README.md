...
(Live flight tracking doesn't need a proxy — it queries
[airplanes.live](https://airplanes.live)'s public API directly from the browser, since
that API sends permissive CORS headers. See the flight tracking note in the root
[`README.md`](../README.md) for why OpenSky Network, tried first, didn't work: it
silently blocks Cloudflare's outbound IP ranges as an anti-scraping measure.)
